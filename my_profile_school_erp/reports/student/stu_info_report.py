...
class StudentProfileReport(models.AbstractModel):
    ...
    def generate_pdf(self, record):
        ...
        elements.append(Paragraph("Academic Information", styles['Heading2']))
        elements.append(Paragraph(f"Classroom: {record.classroom_id.name if record.classroom_id else ''}", styles['Normal']))
        subjects = ", ".join([sub.name for sub in record.subject_id])
        elements.append(Paragraph(f"Subjects: {subjects}", styles['Normal']))
        ...
        elements.append(Paragraph(f"Sequence: {record.sequence or ''}", styles['Normal']))
        elements.append(Spacer(1, 12))
