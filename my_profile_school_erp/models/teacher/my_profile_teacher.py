...
class MyProfileTeacher(models.Model):
    ...
    user_id = fields.Many2one(comodel_name='res.users',readonly=True,default=lambda self: self.env.user)
    ...
    member_type = fields.Selection(related='teacher_id.member_type')
    ...
    def _compute_teacher_id(self):
        for rec in self:
            rec.teacher_id = self.env['teacher.teacher'].search([
